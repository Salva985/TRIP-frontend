...
export default function ActivityDetail() {
  ...
  async function onDelete() {
    ...
    // Keep the tripId BEFORE deleting (DELETE likely returns 204)
    const tripId = s.data?.tripId;
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 space-y-4">
        ...
        <dl className="space-y-2 text-gray-800">
          <div>
            ...
            <dd>{a.tripName || (a.tripId ? `Trip #${a.tripId}` : "—")}</dd>
          </div>
          <div>
        ...
        <div className="flex flex-wrap gap-2 pt-4 border-t">
          ...
          <Link className="px-3 py-2 rounded border border-gray-300 hover:bg-gray-100 transition" to={a.tripId ? `/trips/${a.tripId}` : "/activities"}>
            Back
          </Link>
        </div>
      </div>
